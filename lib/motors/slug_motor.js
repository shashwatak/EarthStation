slug_motor = (function slug_motor () {
  ...
  function parse_motor_response (incoming_bytes, callback) {
        ...
        if (incoming_bytes[response_itor] === 'a'.charCodeAt(0)) {
          ...
          while (incoming_bytes[response_itor] >= '0'.charCodeAt(0) &&
            ...
            if (incoming_bytes[response_itor] !== 0){
              az_voltage.push(String.fromCharCode(incoming_bytes[response_itor++]));
            };
          };
          var az_string = az_voltage.join("");
          console.log("az: " + az_string);
          motor_az = parseInt(az_string, 10) * 2 * Math.PI / 1024;
        }
          ...
          while (incoming_bytes[response_itor] >= '0'.charCodeAt(0) &&
                 incoming_bytes[response_itor] <= '9'.charCodeAt(0)) {
            if (incoming_bytes[response_itor] !== 0) {
              el_voltage.push(String.fromCharCode(incoming_bytes[response_itor++]));
            };
          };
          var el_string = el_voltage.join("");
          console.log("el: " + el_string);
          motor_el = parseInt(el_string, 10) * 2 * Math.PI / 1024;
        }
        else{
          response_itor++;
        };
    ...
    console.log(motor_az + " " + motor_el + " " + motor_status);
    var motor_data = {
      azimuth : motor_az,
      elevation : motor_el,
      status : motor_status
