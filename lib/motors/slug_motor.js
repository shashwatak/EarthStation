	...
	slug_motor = (function slug_motor () {
	  ...
	  function move_az_to (connectionId, azimuth, callback){
		console.log("slug_motors.js: move_az_to()");
	  ...
	  function move_el_to (connectionId, elevation, callback){
		console.log("slug_motors.js: move_el_to()");
	  ...
	  function adjust_az_pwm (track_rad, motor_rad){
		console.log("slug_motors.js: adjust_az_pwm*(");
		if (track_rad < 0) { track_rad = 0; };
		if (track_rad > 359) { track_rad = 359; };
		var diff = track_rad - motor_rad;
		if (diff > Math.PI) {
		  // If arc_angle is more than 180,
		  // don't go the long way around, flip it.
		  //
		  // If the track is at 359 and the motor is at 1
		  // the diff = 358. Rather than move 358 degrees,
		  // 358 - 360 = -2, a much smaller movement.
		  diff -= ( Math.PI * 2 );
		}
		else if (diff < -Math.PI){
		  // If arc_angle is less than -180,
		  // don't go the long way around, flip it.
		  //
		  // If the track is at 1 and motor is at 359,
		  // the diff = -358. Rather than move -358 degrees,
		  // -358 + 360 = 2, a much smaller movement.
		  diff += ( Math.PI * 2 );
		};
	  ...
	  function adjust_el_pwm (track_rad, motor_rad){
		console.log("slug_motors.js: adjust_el_pwm()");
		if (track_rad < 0) { track_rad = 0; };
		var diff = track_rad - motor_rad;
		var diff_mag = Math.abs(diff);
		var diff_sign = 1;
	  ...
	  function update_az_motors (connectionId, az_pwm, callback){
		console.log("slug_motors.js: update_az_motors()");
	  ...
	  function update_el_motors (connectionId, el_pwm, callback){
		console.log("slug_motors.js: update_el_motors()");
	  ...
	  function send_pwn_command (connectionId, pwm_str, callback) {
		console.log("slug_motors.js: send_pwn_command()");
	  ...
	  // lol i just commented out this whole thing for now
	  function get_motor_response (connectionId, num_bytes_expected, callback) {
		console.log("slug_motors.js: get_motor_response()");
	  ...
	  function send_output_data (connectionId, send_bytes, callback) {
		console.log("slug_motors.js: send_output_data() I THINK THE PROBLEM IS HERE");
		...
		chrome.serial.send(connectionId, send_bytes, motor_write_callback);
	  };

	  function parse_motor_response (incoming_bytes, callback) {
		console.log("slug_motors.js: parse_motor_response()");
