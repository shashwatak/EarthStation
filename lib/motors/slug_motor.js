slug_motor = (function slug_motor () {
  ...
  function adjust_pwm (track_rad, motor_rad){
    ...
    if (diff_mag > Math.PI) {
      ...
      diff_sign *= -1;
      diff_mag = ( Math.PI * 2 ) - diff_mag;
    }

    if (diff_mag > Math.PI/10) {
      new_pwm = diff_sign*100;
    }
    else if (diff_mag > Math.PI/20) {
      new_pwm = diff_sign*75;
    }
    else if (diff_mag > Math.PI/40) {
      new_pwm = diff_sign*50;
    }
    else if (diff_mag > Math.PI/80) {
      new_pwm = diff_sign*25;
    }
    else {
      new_pwm = 0;
    }
  ...
  function parse_motor_response (incoming_bytes, callback) {
    ...
    if (response_itor < incoming_length){
      ...
      while (response_itor < incoming_length){
        ...
        if (incoming_bytes[response_itor] === 'a') {
          response_itor++;
          var az_voltage = [];
          while (incoming_bytes[response_itor] > '0' &&
                 incoming_bytes[response_itor] < '9') {
            az_voltage.push(incoming_bytes[response_itor++]);
          }
          var az_string = az_voltage.join();
          motor_az = az_string.parseInt(az_string, 10) * 2 * Math.PI / 1024;
        }
        else if (incoming_bytes[response_itor] === 'e') {
          response_itor++;
          var el_voltage = [];
          while (incoming_bytes[response_itor] > '0' &&
                  incoming_bytes[response_itor] < '9') {
            el_voltage.push(incoming_bytes[response_itor++]);
          }
          var el_string = el_voltage.join();
          motor_el = el_string.parseInt(el_string, 10) * 2 * Math.PI / 1024;
        };
      };
    }
    if (callback) {
