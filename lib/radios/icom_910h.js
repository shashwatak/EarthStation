...
icom_910h = (function() {
  ...
  function set_icom_frequency (connectionId, frequency, callback){
	...
	if(callback) {console.log("SET freq WORKED");};
...
/*
  function get_icom_frequency (connectionId, callback) {
    ...
    // Use the callback to use the date
	console.log("GETTING FREQ");
    var serial_out_data = build_icom_packet (ICOM_FREQ_POLL, []);
      // Serialize bytes from command hex and data array
    send_icom_command (connectionId, serial_out_data, callback);
	if(callback) {console.log("GET freq WORKED");};
      // Encapsulated SerialWrite
  };
*/
  ...
  function set_icom_sub_band (connectionId, callback) {
    ...
    var serial_out_data = build_icom_packet(ICOM_MAINSUB_CMD, ICOM_SUB_SCMD);
      // Serialize bytes from command hex and data array
    send_icom_command (connectionId, serial_out_data, callback);
	if(callback) {console.log("SET sub freq WORKED");};
      // Encapsulated SerialWrite
  };

  function set_icom_main_band (connectionId, callback) {
    console.log("SETTING ICOM MAIN BAND! callback is: " + callback);
    ...
    send_icom_command (connectionId, serial_out_data, callback);
	if(callback) {
		console.log("SET main band WORK");
	}
      // Encapsulated SerialWrite
  };
  ...
  function build_icom_packet (hex_command, hex_data) {
	...
	console.log("HEX LENGTH: " + hex_data.length);
  ...
  console.log("sending command");
    ...
    if (connectionId > 0){
      ...
      var icom_write_callback = function (write_info) {
        var bytes_written = write_info.bytesSent;
      ...
      //setTimeout(function() {chrome.serial.send(connectionId, serial_out_data, icom_write_callback);}, 1000); 
	  chrome.serial.send(connectionId, serial_out_data, icom_write_callback);
	  console.log("SENT COMMAND" + serial_out_data );

	  
	  //need to fix this for return of callback
  ...
  function get_icom_response (connectionId, num_bytes_expected, callback) {
    ...
    var blocking_serial_read_callback = function (read_info) {
      ...
      if (read_info.bytesRead === 0) { //No bytes were read
        ...
        if (bytes_remaining > 0) {
		  ...
		  console("NO BYTES REMAINING, READ AGAIN");
		  chrome.serial.onReceive(connectionId,blocking_serial_read_callback);
        }
	...
	//chrome.serial.onReceive.onReceive(blocking_serial_read_callback);

  };
  */
  /*
  //start fixing this function as new get_response
  
  //Whenever we get data from the radio
...
var onReceiveCallback = function(info) {
      ...
      var serial_in_view = new Uint8Array(info.data);
		...
		if (serial_in_view[serial_in_view.length-1] == ICOM_EOM){
		console.log("EOM");
		}
		//console.log("INCOMING BYTES IS " + incoming_bytes);
		...
		if (!parse_results) {
           ...
           // get_icom_response (info.connectionId, ICOM_BASE_MSG_SIZE, callback);
            ...
            // Our blocking read has come to an end, if the
            // callback exists, call it. We are done here!
			if(parse_results === parseInt(parse_results)) freqList.push(parse_results);
...
*/
  ...
  function get_main_frequency (connectionId, callback) {
	console.log("get_main_frequency, callback is "+callback);
  ...
  function set_main_frequency (connectionId, frequency, callback) {
    console.log("SETTING main FREQUENCY, ON TOP + callback: " + callback);
    set_icom_main_band (connectionId, function (result){
  ...
  console.log("SETTING sub FREQUENCY, ON TOP");
    set_icom_sub_band (connectionId, function (result){ // This function sets the ICOM to Sub Band operation mode
      console.log("what is result : " + result);
	  if (result !== false){
        set_icom_frequency (connectionId, frequency, callback);
      };
