...
self.addEventListener('message', function(e) {
  if (e.data) {
    var satrec = e.data;
    var ecf_coords = propagate_current_orbit(satrec);
    self.postMessage({  cmd      : 'update',
                        sat_item :
                        { satnum     : satrec.satnum,
                          ecf_coords   : ecf_coords
                        }
                     });
  }
}, false);
...
function propagate_current_orbit (satrec) {
  ...
  while (itor < orbital_period){
    ...
    satellite_ecf_coords.push(position_ecf);
    itor++;
  }
  return satellite_ecf_coords;
};
