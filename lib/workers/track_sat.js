...
function track_sats (time) {
  for (var satnum in sat_table){
    if (sat_table.hasOwnProperty(satnum)) {
      ...
      var gmst = satellite.gstime_from_date (time.year, time.month, time.day, time.hour, time.minute, time.second);
      var r_v = satellite.propagate(satrec, time.year, time.month, time.day, time.hour, time.minute, time.second);
      ...
      var geodetic_coords = satellite.eci_to_geodetic (position_eci, gmst);
      var lat_long_alt = [
        satellite.degrees_lat (geodetic_coords[1]),
        satellite.degrees_long (geodetic_coords[0]),
        geodetic_coords[2]
      ];
